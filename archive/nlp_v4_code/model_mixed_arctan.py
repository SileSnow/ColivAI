...
==================================
sin/cos 绝对 PE + 可学习 arctan + 1/(1+d²) 相对偏置
...
class MixedPEAttention(nn.Module):
    ...
    偏置公式:
      rel_bias(d) = w · arctan(v · d) + 1/(1 + (τ·d)²)
    ...
    def _compute_bias(self, L: int) -> torch.Tensor:
        """
        计算可学习偏置矩阵: arctan + 1/(1+d²)
        返回: (nhead, L, L)
        """
        d = self.distances[:L, :L]
        d_abs = torch.abs(d)

        # 局部项: 1/(1 + (τ·d)²) — arctan 的导数，天然安全
        tau = torch.exp(self.log_tau).view(self.nhead, 1, 1)
        local_term = 1.0 / (1.0 + (tau * d_abs.unsqueeze(0)) ** 2)

        # arctan 项: 长距离多项式衰减梯度，不饱和
        v = self.v_param.view(self.nhead, 1, 1)
        w = self.w_param.view(self.nhead, 1, 1)
        arctan_term = torch.arctan(v * d_abs.unsqueeze(0))

        return w * arctan_term + local_term
