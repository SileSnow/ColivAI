...
class TransformerCoTWithCache(nn.Module):
    ...
    def generate_batch(self, prompt: str, tokenizer: CharTokenizer,
        ...
        torch.cat 每次创建新 tensor，base_kv 不会被修改，无需深拷贝。
        """
        self.eval()
        device = next(self.parameters()).device
        ...
        # 处理 prompt 一次，缓存 KV
        _, base_kv = self(prompt_ids, return_kv=True)
        last_token = prompt_ids[:, -1:]

        results = []
        for _ in range(n_samples):
            past_kv = base_kv  # 共享！torch.cat 不修改原始 tensor
            current = last_token
            generated_ids = []
...
class ResponseBuffer:
    ...
    def __init__(self, path: str = "/root/airesearch/v3/buffer.pt",
        ...
        self.buffer = []          # [(prompt_tokens, resp_tokens, reward, gt), ...]
        self.dirty = False
        self._load()
    ...
    def add(self, prompt_tokens: List[int], resp_tokens: List[int], reward: float, gt: int):
        self.buffer.append((prompt_tokens, resp_tokens, reward, gt))
    ...
    def add_batch(self, prompts_tokens, responses_list, rewards, gts):
        """批量添加"""
        for p_tokens, r_tokens, r, gt in zip(prompts_tokens, responses_list, rewards, gts):
            self.buffer.append((p_tokens, r_tokens, r, gt))
        if len(self.buffer) > self.max_size:
            self.buffer.sort(key=lambda x: x[2], reverse=True)
            self.buffer = self.buffer[:self.max_size]
        self.dirty = True
    ...
    def sample(self, n: int) -> List[Tuple]:
        ...
        # 返回格式: [(prompt_tokens, resp_tokens, reward, gt), ...]
        # 按 reward 加权采样
...
def train_rl_epoch(model, tokenizer, dataset, optimizer, device,
    ...
    for batch_idx, idx in enumerate(indices):
        ...
        responses = []
        all_resp_tokens = []
        response_prompts = []  # 每个回答对应的 prompt 字符串（用于奖励计算）
        response_prompt_tokens = []  # 每个回答对应的 prompt tokens（用于 log_probs）
        ...
        for p_tokens_buf, r_tokens, r, gt_buf in replay_samples:
            responses.append(tokenizer.decode(r_tokens))
            all_resp_tokens.append(r_tokens)
            # 用 decode 后的 prompt 字符串做奖励计算（regex 能跳过 [BOS]/[EOS] 文本）
            response_prompts.append(tokenizer.decode(p_tokens_buf))
            # 直接用 buffer 存的原生 prompt_tokens（含 BOS/EOS），不做 decode-encode 往返！
            response_prompt_tokens.append(p_tokens_buf)
        ...
        # 直接使用 response_prompt_tokens（已含 BOS/EOS，避免 decode-encode 往返）
        response_buffer.add_batch(
            response_prompt_tokens, all_resp_tokens, rewards, response_gts
        )
        ...
        for i in range(len(all_resp_tokens)):
            if len(all_resp_tokens[i]) == 0:
                continue
            # 每个样本用自己对应的 prompt_tokens（回放样本直接用 buffer 存的）
...
def main():
    ...
    # === GRPO 超参 ===
    RL_KL_BETA      = 0.05   # GRPO 无偏 KL 惩罚系数（调高防止 KL 爆表）
    ...
    if response_buffer.size < COLD_START_SIZE:
        ...
        while response_buffer.size < COLD_START_SIZE:
            ...
            prompt_tokens_enc = tokenizer.encode(prompt, add_special=True)
            response_buffer.add(prompt_tokens_enc, tokens, reward, gt)
    ...
    if len(sft_data) > 0:
        ...
        for sft_epoch in range(SFT_MAX_EPOCHS):
            ...
            # 早停
            loss_improved = avg_loss < best_loss
            if loss_improved:
                best_loss = avg_loss
                no_improve = 0
            else:
